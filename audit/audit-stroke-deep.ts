...
console.log(`=== AUDIT BATCH 3: STROKE_BRANDS (${STROKE_BRANDS.length} Brands) ===`);

let passed = 0;
const issues: Array<{ id: string; name: string; error: string }> = [];

for (let i = 0; i < STROKE_BRANDS.length; i++) {
  const brand = STROKE_BRANDS[i];
...
console.log(`Results: ${passed}/${STROKE_BRANDS.length} passed.`);
if (issues.length > 0) {
  console.log('Issues found:');
  issues.forEach(iss => console.log(`  - [${iss.id}] ${iss.name}: ${iss.error}`));
} else {
  console.log('✅ Batch 3 STROKE_BRANDS 100/100 verified clean!');
}

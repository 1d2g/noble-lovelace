...
console.log(`=== AUDIT BATCH 2: TILT_BRANDS (${TILT_BRANDS.length} Brands) ===`);

let passed = 0;
const issues: Array<{ id: string; name: string; error: string }> = [];

for (let i = 0; i < TILT_BRANDS.length; i++) {
  const brand = TILT_BRANDS[i];
...
console.log(`Results: ${passed}/${TILT_BRANDS.length} passed.`);
if (issues.length > 0) {
  console.log('Issues found:');
  issues.forEach(iss => console.log(`  - [${iss.id}] ${iss.name}: ${iss.error}`));
} else {
  console.log('✅ Batch 2 TILT_BRANDS 100/100 verified clean!');
}

...
console.log(`=== AUDIT BATCH 5: RADIAL_BRANDS (${RADIAL_BRANDS.length} Brands) ===`);

let passed = 0;
const issues: Array<{ id: string; name: string; error: string }> = [];

for (let i = 0; i < RADIAL_BRANDS.length; i++) {
  const brand = RADIAL_BRANDS[i];
...
console.log(`Results: ${passed}/${RADIAL_BRANDS.length} passed.`);
if (issues.length > 0) {
  console.log('Issues found:');
  issues.forEach(iss => console.log(`  - [${iss.id}] ${iss.name}: ${iss.error}`));
} else {
  console.log('✅ Batch 5 RADIAL_BRANDS 100/100 verified clean!');
}
